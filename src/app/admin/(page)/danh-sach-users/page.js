    ...
    <PageContainer title="Danh sách người dùng" description="Danh sách tất cả người dùng trong hệ thống">
      ...
      >
        <DialogTitle id="confirm-dialog-title">
          {confirmDialog.action === "lock" ? "Xác nhận khóa tài khoản người dùng" : "Xác nhận mở khóa tài khoản người dùng"}
        </DialogTitle>
        <DialogContent>
          <DialogContentText id="confirm-dialog-description">
            Bạn có chắc chắn muốn {confirmDialog.action === "lock" ? "khóa" : "mở khóa"} tài khoản người dùng {" "}
            <strong>{confirmDialog.user?.name}</strong> ({confirmDialog.user?.email})?
            {confirmDialog.action === "lock" && (
              <div style={{ marginTop: "10px" }}>
                <span style={{ color: "red", fontWeight: "bold" }}>
                  ⚠️ Lưu ý: Người dùng sẽ không thể đăng nhập vào hệ thống sau khi bị khóa.
                </span>
              </div>
            )}
          </DialogContentText>
